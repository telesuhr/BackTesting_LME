"""
LME Copper 15分足バックテストスクリプト

使用方法:
    python run_backtest_15min.py
"""
import logging
...
def load_15min_data(target_date='2025-11-10', ric_code='CMCU3', interval='15min'):
    ...
    try:
        ...
        logger.info(f"15分足データ読み込み成功: {target_date}")
...
def print_backtest_summary(result):
    """バックテスト結果のサマリーを表示"""
    print("\n" + "=" * 80)
    print("ボリンジャーバンド戦略 - バックテスト結果（15分足）")
    print("=" * 80)
...
def print_comparison_table():
    """1分足、5分足、15分足の比較表を表示"""
    print("\n" + "=" * 80)
    print("時間軸別比較サマリー（2025-11-10）")
    print("=" * 80)
    print(f"\n{'指標':<25} {'1分足':<20} {'5分足':<20} {'15分足':<20}")
    print("-" * 80)
    print(f"{'トレード数':<25} {'17回':<20} {'4回':<20} {'後で更新':<20}")
    print(f"{'手数料控除前損益':<25} {'+3,111円':<20} {'+9,657円':<20} {'後で更新':<20}")
    print(f"{'取引コスト':<25} {'-503,842円':<20} {'-151,693円':<20} {'後で更新':<20}")
    print(f"{'純損益':<25} {'-500,731円':<20} {'-142,036円':<20} {'後で更新':<20}")
    print(f"{'総リターン':<25} {'-50.07%':<20} {'-14.20%':<20} {'後で更新':<20}")
    print("=" * 80)
...
def main():
    ...
    logger.info("=" * 60)
    logger.info(f"LME Copper 15分足バックテスト: {target_date}")
    logger.info("=" * 60)
