"""
LME Copper ボリンジャーバンド戦略 バックテストスクリプト

使用方法:
    python run_backtest_bollinger.py
"""
import logging
...
def main():
    """メイン処理"""
    logger.info("=" * 60)
    logger.info("LME Copper ボリンジャーバンド戦略")
    logger.info("=" * 60)
    ...
    strategy = BollingerBandsStrategy(
        ...
        bb_std=2.0,             # 標準偏差の倍率
        take_profit_pct=0.015,  # 1.5%利益確定
        stop_loss_pct=0.015,    # 1.5%損切り
    ...
    logger.info(f"  利益確定:             {strategy.take_profit_pct:.2%}")
    logger.info(f"  損切り:               {strategy.stop_loss_pct:.2%}")
