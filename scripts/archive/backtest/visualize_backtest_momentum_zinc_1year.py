"""
モメンタム戦略の1年間バックテスト可視化スクリプト（15分足）

使用方法:
    python visualize_backtest_momentum_zinc_1year.py
"""
import logging
...
def load_1year_data(start_date='2024-11-11', end_date='2025-11-11', ric_code='CMZN3', interval='15min'):
...
def visualize_momentum_backtest(data, result, strategy, strategy_name='momentum', output_file=None):
    """
    1年間のモメンタム戦略バックテスト結果を可視化
    ...
    if output_file is None:
        ...
        output_file = f'{strategy_name}_backtest_zinc_1year_100mt_{timestamp}.png'
    ...
    # === 上部: 価格チャートと移動平均線（サンプリング） ===
    # データ量が多いので24時間ごとにサンプリング
    sampled_data = data.iloc[::96]  # 15分×96 = 24時間分ごと

    # 移動平均を計算（サンプリング前のデータで）
    fast_ma, slow_ma = strategy.calculate_moving_averages(data)
    sampled_fast_ma = fast_ma.iloc[::96]
    sampled_slow_ma = slow_ma.iloc[::96]

    ax1.plot(sampled_data.index, sampled_data['close'], color='black', linewidth=1, alpha=0.7, label='Close Price')
    ax1.plot(sampled_fast_ma.index, sampled_fast_ma, color='blue', linewidth=1.5, alpha=0.7, label=f'Fast MA({strategy.fast_period})')
    ax1.plot(sampled_slow_ma.index, sampled_slow_ma, color='red', linewidth=1.5, alpha=0.7, label=f'Slow MA({strategy.slow_period})')
    ...
    # タイトルに戦略パラメーターを含める
    title = (f'LME Zinc - Momentum Strategy (15分足, {strategy.fixed_position_size:.0f}MT, '
            f'MA{strategy.fast_period}/{strategy.slow_period}, 2024-11-11 ~ 2025-11-11)')
...
Fast MA: {strategy.fast_period} periods
Slow MA: {strategy.slow_period} periods
...
def main():
    ...
    logger.info("=" * 60)
    logger.info(f"モメンタム戦略 亜鉛1年間可視化（15分足, 100MT）")
    logger.info("=" * 60)

    # データ読み込み
    data = load_1year_data(start_date=start_date, end_date=end_date, ric_code='CMZN3', interval='15min')
    ...
    # 可視化
    output_file = visualize_momentum_backtest(data, result, strategy, strategy_name='momentum')
