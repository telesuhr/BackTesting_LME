"""
ボリンジャーバンド戦略の1年間バックテスト可視化スクリプト（15分足）

使用方法:
    python visualize_backtest_zinc_1year.py
"""
import logging
...
def visualize_1year_backtest(data, result, strategy, strategy_name='bollinger', output_file=None):
    ...
    # タイトルに戦略パラメーターを含める
    title = (f'LME Zinc - Bollinger Bands Strategy (15分足, {strategy.fixed_position_size:.0f}MT, '
...
def main():
    ...
    logger.info("=" * 60)
    logger.info(f"ボリンジャーバンド戦略 亜鉛1年間可視化（15分足, 100MT）")
    logger.info("=" * 60)
    ...
    # 可視化
    output_file = visualize_1year_backtest(data, result, strategy, strategy_name='bollinger')
