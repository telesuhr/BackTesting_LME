"""
ボリンジャーバンド戦略の可視化スクリプト（15分足）

使用方法:
    python visualize_trades_15min.py
"""
import logging
...
def load_15min_data(target_date='2025-11-10', ric_code='CMCU3', interval='15min'):
    """データベースから15分足データを読み込む"""
    try:
...
def visualize_bollinger_strategy(data, result, output_file='bollinger_trades_15min.png'):
    """
    ボリンジャーバンド戦略を可視化
    ...
    # 価格推移
    ax.plot(data.index, data['close'], label='Close Price', color='black', linewidth=2, zorder=5)

    # ボリンジャーバンド（2σ）
    ax.plot(data.index, upper_2sigma, label='Upper 2σ', color='red', linewidth=1.5, linestyle='--', alpha=0.7)
    ax.plot(data.index, middle, label='Middle (MA20)', color='blue', linewidth=2, alpha=0.7)
    ax.plot(data.index, lower_2sigma, label='Lower 2σ', color='green', linewidth=1.5, linestyle='--', alpha=0.7)
    ...
    for trade in trades:
        ...
        if position == 1:  # ロング
            ...
            ax.scatter(entry_time, entry_price, color='green', marker='^',
                      s=200, zorder=10, edgecolors='black', linewidths=2)
        ...
        ax.text(mid_time, mid_price, pnl_text, fontsize=10, fontweight='bold',
    ...
    ax.set_title('LME Copper - Bollinger Bands Strategy (15分足, 2025-11-10)',
    ...
    custom_lines = [
        Line2D([0], [0], marker='^', color='w', markerfacecolor='green',
               markersize=12, markeredgecolor='black', linewidth=2, label='ロングエントリー'),
        Line2D([0], [0], marker='v', color='w', markerfacecolor='red',
               markersize=12, markeredgecolor='black', linewidth=2, label='ショートエントリー'),
        Line2D([0], [0], marker='^', color='w', markerfacecolor='blue',
               markersize=12, markeredgecolor='black', linewidth=2, label='利食い（ロング）'),
        Line2D([0], [0], marker='v', color='w', markerfacecolor='blue',
               markersize=12, markeredgecolor='black', linewidth=2, label='利食い（ショート）'),
    ...
    print(f"\n総損益: {total_pnl:,.0f}円")
    print(f"平均損益: {total_pnl/len(trades):,.0f}円/回")
    print("=" * 80)
...
def main():
    ...
    logger.info("=" * 60)
    logger.info(f"ボリンジャーバンド戦略 可視化（15分足）: {target_date}")
    logger.info("=" * 60)

    # データ読み込み
    data = load_15min_data(target_date=target_date, ric_code='CMCU3', interval='15min')
    ...
    # 可視化
    visualize_bollinger_strategy(data, result, output_file='bollinger_trades_15min_20251110.png')

    print("\n可視化が完了しました。")
    print("生成されたファイル: bollinger_trades_15min_20251110.png")
