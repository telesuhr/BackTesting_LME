"""
LME亜鉛（2024-11-11 ~ 2025-11-11）のデータを取得
"""
import logging
...
def fetch_by_month(client, start_date_str, end_date_str, ric_code='CMZN3'):
    """月単位でデータを取得"""
    from datetime import datetime, timedelta
    import time
...
def main():
    ...
    end_date = '2025-11-01'  # 11月分はスキップ

    logger.info("=" * 60)
    logger.info(f"LME亜鉛（CMZN3）データ取得: {start_date} ～ {end_date}")
    logger.info("=" * 60)
    ...
    try:
        ...
        # 月単位でデータ取得
        fetch_by_month(client, start_date, end_date, ric_code='CMZN3')
