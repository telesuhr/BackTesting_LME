"""
LME Copper 1年間バックテストスクリプト（モメンタム戦略、15分足）

使用方法:
    python run_backtest_momentum_1year.py
"""
import logging
...
def print_backtest_summary(result, start_date, end_date):
    """バックテスト結果のサマリーを表示"""
    print("\n" + "=" * 80)
    print(f"モメンタム戦略（移動平均クロスオーバー） - 1年間バックテスト結果（15分足）")
...
def print_trade_summary(trades, max_display=10):
    ...
    reverse_cross_count = sum(1 for t in trades if t['exit_reason'] == 'reverse_cross')
    ...
    print(f"  逆クロス: {reverse_cross_count}回 ({reverse_cross_count/len(trades)*100:.1f}%)")
...
def main():
    ...
    logger.info("=" * 60)
    logger.info(f"LME Copper 1年間バックテスト（モメンタム戦略、15分足）")
    ...
    logger.info("\n戦略パラメータ:")
    logger.info(f"  短期移動平均:         {strategy.fast_period}期間")
    logger.info(f"  長期移動平均:         {strategy.slow_period}期間")
    logger.info(f"  利食い:              {strategy.take_profit_pct:.1%}")
    logger.info(f"  損切り:              {strategy.stop_loss_pct:.1%}")
