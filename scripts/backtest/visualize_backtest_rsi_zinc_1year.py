"""
RSI逆張り戦略の1年間バックテスト可視化スクリプト（15分足）

使用方法:
    python visualize_backtest_rsi_1year.py
"""
import logging
...
def visualize_1year_backtest(data, result, strategy, strategy_name='bollinger', output_file=None):
    """
    1年間のバックテスト結果を可視化
    ...
    # タイトルに戦略パラメーターを含める
    title = (f'LME Zinc - RSI Reversal Strategy (15分足, {strategy.fixed_position_size:.0f}MT, '
            f'RSI({strategy.rsi_period}), 2024-11-11 ~ 2025-11-11)')
...
def main():
    ...
    logger.info("=" * 60)
    logger.info(f"RSI逆張り戦略 1年間可視化（15分足, 100MT）")
    logger.info("=" * 60)
    ...
    # 可視化
    output_file = visualize_1year_backtest(data, result, strategy, strategy_name='rsi')
