"""
1分足データを5分足にリサンプルしてデータベースに保存

使用方法:
    python resample_to_5min.py
"""
import logging
import sys
import os
...
def save_to_database(df, ric_code='CMCU3', interval='5min'):
    """5分足データをデータベースに保存"""
    try:
        conn = psycopg2.connect(
            host='localhost',
...
def main():
    ...
    # データベースに保存
    success = save_to_database(df_5min, ric_code='CMCU3', interval='5min')

    if success:
        logger.info("=" * 60)
        logger.info("リサンプル完了")
        logger.info("=" * 60)
        logger.info(f"5分足データ数: {len(df_5min):,}行")
        logger.info(f"期間: {df_5min.index.min()} ～ {df_5min.index.max()}")
    else:
